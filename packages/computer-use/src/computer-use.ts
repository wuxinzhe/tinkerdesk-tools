/**
 * computer-use.ts — computer_use 外置工具（实现 IAgentTool，与内建工具同构）
 *
 * 调用本机 cua-driver（macOS/Windows/Linux 桌面后台控制）的完整工具：
 ...
 * 单工具 + action 判别（24 类动作）。自包含 Node 模块——不依赖 TinkerDesk 内部类。
 *
 * 出口：export class ComputerUse——实现 getSchema/execute/check（IAgentTool 契约）。
 * 类型定义归位 ./types.ts。
 */

import { CuaDriverClient, CuaDriverUnavailableError } from './lib/cua-driver-client'
import {
...
export class ComputerUse {
  ...
  /** IAgentTool：check——可用性（本工具自包含；cua-driver 运行时可用性由 dispatch 失败兜底） */
  check(): boolean {
    return true
  }
  ...
  private static readonly ACTION_TO_BROWSER_TOOL: Record<string, string> = {
  ...
  private static readonly BROWSER_ALLOWED_FIELDS: Record<string, string[]> = {
  ...
  /** 按 action 分发调用 cua-driver client（原 execute——保留 ActionResult 契约） */
  private static async run(toolCall: ToolCall): Promise<ActionResult> {
    ...
    try {
      ...
      return {
        ok: false,
        error: `computer_use backend unavailable: ${ComputerUse.errMsg(e)}`,
    ...
    try {
      const result = await ComputerUse.dispatch(client, action, args)
      return { ok: true, output: result }
    } catch (e) {
      if (e instanceof CuaDriverUnavailableError) {
        return { ok: false, error: e.message }
      }
      return { ok: false, error: `${action} failed: ${ComputerUse.errMsg(e)}` }
    } finally {
  ...
  private static async dispatch(client: CuaDriverClient, action: string, args: ToolCallArgumentBag): Promise<string> {
    ...
    switch (action) {
      case 'capture': {
        ...
        const maxElements = ComputerUse.coerceMaxElements(args.max_elements)
        // 解析目标 pid/window_id：显式传参优先；否则 list_windows 匹配 app 或取第一个窗口
        let pid = args.pid
        let windowId = args.window_id
        if (pid === undefined || windowId === undefined) {
          const windows = await ComputerUse.listWindowsParsed(client)
        ...
        return ComputerUse.captureResponse(res, maxElements)
      }
      ...
      case 'focus_app': {
        ...
        return ComputerUse.maybeFollowCapture(client, res, captureAfter)
      }
      ...
      default: {
        const browserTool = ComputerUse.ACTION_TO_BROWSER_TOOL[action]
        if (browserTool) {
          const callArgs: ToolCallArgumentBag = {}
          for (const field of ComputerUse.BROWSER_ALLOWED_FIELDS[browserTool]) {
        ...
        return ComputerUse.dispatchInput(client, action, args, captureAfter)
      }
    }
  }

  /** 输入类 action（click 系列/drag/scroll/type/key/set_value）——_dispatch 尾部 */
  private static async dispatchInput(
  ...
  private static async listWindowsParsed(client: CuaDriverClient): Promise<WindowInfo[]> {
    const res = await client.callTool('list_windows', {})
   ...
   */
  private static captureResponse(res: MCPToolResult, _maxElements: number): string {
  ...
  private static async maybeFollowCapture(client: CuaDriverClient, res: MCPToolResult, captureAfter: boolean): Promise<string> {
    const base = ComputerUse.clientExtractTextSafe(res)
    if (captureAfter) {
      return ComputerUse.dispatch(client, 'capture', {})
    }
    return base
  }

  private static coerceMaxElements(v: unknown): number {
    const n = Number(v)
  ...
  private static parseJsonArray(res: MCPToolResult): any[] {
    const text = ComputerUse.clientExtractTextSafe(res)
    try {
  ...
  /** 工具内文本提取 */
  private static clientExtractTextSafe(res: MCPToolResult): string {
    const parts: string[] = []
